...
# from ml.models.segmentation.segmentation import UNet
import segmentation_models_pytorch as smp
from ml.models.segmentation.utils import CLASSES_IDS_TO_LABELS_DICT, CLASSES_IDS_TO_COLORS_DICT
...
def model():
    ...
    R = img_data[0::4].reshape(height, width)
    G = img_data[1::4].reshape(height, width)
    B = img_data[2::4].reshape(height, width)
    ...
    # new_preds_unique = np.unique(preds.ravel())
    for j in range(0, preds.shape[0]):
        for k in range(0, preds.shape[1]):
            new_preds[j][k] = CLASSES_IDS_TO_COLORS_DICT[preds[j][k]]

    # patches_pred = [mpatches.Patch(color=list(map(lambda x: x / 255, CLASSES_IDS_TO_COLORS_DICT[value])),
    #                                label=f"{CLASSES_IDS_TO_LABELS_DICT[value]}") for value in new_preds_unique]
